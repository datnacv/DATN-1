package com.example.AsmGD1.specification;
...
public class PhieuGiamGiaSpecification {
    ...
    public static Specification<PhieuGiamGia> filter(String search, String type, String status, LocalDate fromDate, LocalDate toDate) {
        return (root, query, cb) -> {
            ...
            if (status != null) {
                ...
                switch (status) {
                    case "upcoming" -> predicate = cb.and(predicate, cb.greaterThan(root.get("ngayBatDau"), now.toLocalDate()));
                    case "active" -> predicate = cb.and(predicate,
                            cb.lessThanOrEqualTo(root.get("ngayBatDau"), now.toLocalDate()),
                            cb.greaterThanOrEqualTo(root.get("ngayKetThuc"), now.toLocalDate()));
                    case "expired" -> predicate = cb.and(predicate, cb.lessThan(root.get("ngayKetThuc"), now.toLocalDate()));
                }
            }
